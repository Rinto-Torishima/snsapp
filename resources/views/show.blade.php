    ...
    @else
        <div class="form">
            ...
            <form action="{{ route('cmt.store') }}" method="POST">
                ...
                <input id="ftop" class="form-control" value="{{ old('comment_name') }}" type="text" name="comment_name"
                    placeholder="投稿者" required>
                <textarea id="ftop" rows="3" class="form-control" name="comment_message" placeholder="投稿内容"
                    required>{{ old('comment_message') }}</textarea>
