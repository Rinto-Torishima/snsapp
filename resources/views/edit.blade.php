    ...
    @if ($idd === $topic->user_id)
        <div class="form">
            <form action="{{ route('topics.update', $topic->id) }}" method="POST">
                {{ csrf_field() }}
                {{ method_field('put') }}
                <input id="ftop" class="form-control" type="text" name="name" placeholder="名前" value="{{ $topic->name }}"
                    required>
                <textarea id="ftop" class="form-control" name="content" rows="3" placeholder="投稿内容"
